...
namespace CP_SDK_BS.UI
{
        ...
        where t_Base : ViewController<t_Base>
    {
        ...
        public override void ShowModal(CP_SDK.UI.IModal p_Modal)
        {
            ...
            {
                CP_SDK.ChatPlexSDK.Logger.Error($"[CP_SDK_BS.UI][ViewController<{typeof(t_Base).FullName}>.ShowModal] Null or invalid parented modal, not showing!");
                return;
            }
            ...
            if (!p_Modal.gameObject.activeSelf)
            {
                ...
                {
                    CP_SDK.ChatPlexSDK.Logger.Error($"[CP_SDK_BS.UI][ViewController<{typeof(t_Base).FullName}>.ShowModal] Error:");
        ...
        public override void CloseModal(CP_SDK.UI.IModal p_Modal)
        {
            if (!p_Modal || p_Modal.RTransform.parent != m_ModalContainer.RTransform)
            {
                CP_SDK.ChatPlexSDK.Logger.Error($"[CP_SDK_BS.UI][ViewController<{typeof(t_Base).FullName}>.CloseModal] Null or invalid parented modal, not closing!");
                return;
            }
            ...
            if (p_Modal.gameObject.activeSelf)
            {
                ...
                {
                    CP_SDK.ChatPlexSDK.Logger.Error($"[CP_SDK_BS.UI][ViewController<{typeof(t_Base).FullName}>.CloseModal] Error:");
                    CP_SDK.ChatPlexSDK.Logger.Error(l_Exception);
                }
