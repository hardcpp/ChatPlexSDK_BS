...
namespace CP_SDK_BS.UI.DefaultComponentsOverrides.Subs
{
    ...
    internal class SubFloatingPanelMover : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            var l_RTransform            = m_FloatingPanel.RTransform;
            var l_ControllerTransform   = m_GrabbingController.transform;

            var l_RealPosition = l_ControllerTransform.TransformPoint(m_GrabPosition);
            var l_RealRotation = l_ControllerTransform.rotation * m_GrabRotation;

            l_RTransform.position = Vector3.Lerp(l_RTransform.position,       l_RealPosition, 10f * Time.unscaledDeltaTime);
            l_RTransform.rotation = Quaternion.Slerp(l_RTransform.rotation,   l_RealRotation,  5f * Time.unscaledDeltaTime);
        }
